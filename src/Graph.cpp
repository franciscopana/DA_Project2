...
void Graph::prim_generate_MST(){
    ...
    while (!pq.empty()) {
        ...
        for (Edge *edge: dest->getEdges()) {
            ...
            if (!nodes[otherId]->isVisited() && !edge->isSelected()) {
                pq.push(edge);
            }
        }
...
pair<double, vector<unsigned int>> Graph::TSP_CheapestInsertion() {
    for(auto node : nodes){
        node->setVisited(false);
    }
    ...
    while (path.size() < nodes.size()) {
        ...
        for (unsigned int k = 0; k < nodes.size(); k++) {
            if (!nodes[k]->isVisited()) {
                double CheapestDist = std::numeric_limits<double>::max();
                for (unsigned int j : path) {
                    auto edge = nodes[j]->getEdgeTo(nodes[k]);
                    if(edge == nullptr) continue;
                    double dist = edge->getDistance();
                    if (dist < CheapestDist) {
                        CheapestDist = dist;
                        r = k;
                    }
                }
                if (CheapestDist < minCost) {
                    minCost = CheapestDist;
                }
            }
        }
